# recommendation_chatbot.py - 대화형 공지 추천 챗봇
import os
import re
import json
import logging
...
# ===== 고급 검색 로직 (rag_chat_api.py에서 가져온 함수들) =====
def _basic_tokens(s: str) -> List[str]:
    """기본 토큰 추출"""
    return [t for t in re.findall(r"[0-9A-Za-z가-힣]+", (s or "").lower()) if t]

def _clean_query(q: str) -> str:
    """불용어 제거하여 쿼리 정리 - 향상된 버전"""
    if not q or not q.strip():
        return ""

    try:
        # 고급 텍스트 전처리 사용
        from src.uosai.preprocess.text_process import quick_nouns
        nouns = quick_nouns(q)

        # 명사 추출이 성공한 경우, 중복 제거하고 의미있는 명사만 반환
        if nouns:
            # 중복 제거하면서 순서 유지
            unique_nouns = []
            seen = set()
            ...
            # 최대 3개의 핵심 명사만 사용 (쿼리 과적합 방지)
            result = " ".join(unique_nouns[:3])
            if result.strip():
                return result

    except Exception as e:
        # 고급 전처리 실패 시 기존 방식 사용
        pass
...
def rerank_documents(query: str, docs: List[Document]) -> List[Tuple[Document, float]]:
    """입력 쿼리와 문서들을 Cohere Reranker로 재정렬"""
...
def generate_final_recommendation(requirements: Dict[str, Any], notice: Dict[str, Any], conversation_history: List[Dict[str, str]]) -> str:
    ...
    prompt = f"""사용자가 다음과 같이 질문했습니다: "{conversation_summary}"
...
6. 마지막에 링크: {notice.get('link')} 제공

**예시 톤:**
"100주년기념도서관에서 매주 목요일마다 독립영화를 무료로 상영하고 있어요. 별도 신청 없이 누구나 참여할 수 있고..."
